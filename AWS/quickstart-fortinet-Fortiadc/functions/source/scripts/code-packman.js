...
class Packman {
    ...
    async deleteAllExcept(location, onDir, options = {}) {
        if (!onDir) {
            console.error('<onDir> must be provided.');
            return false;
        }
        ...
        await this.execCmd(`find ${realPath} -type d -not \\( -name 'fortiadc-autoscale-*' -or -name 'uuid' -or  -name 'lib' -or -name 'bin' \\) -exec rm -rf {} + `, onDir, options);
    }
